            ...
            else
            {
                ...
                while (!ifs.eof())
                {
                    ...
                    if (action == "rent")
                    {
                        ...
                        //! 檢查該車站的指定車種是否存在
                        // todo 了解heap功用?
                        // 藉由上方，去該station把指定車種的heap_ptr拿出來
                        // heap_ptr[100]=>heap size
                    ...
                    else if (action == "return") //! 計算revenue
                    {
                        ...
                        { // not i <= rent_num, cuz rent_num will be the actual value +1 (++ at the last time)
                            //* 一一從rent_list中搜尋user rent紀錄
                            if (user_id == rent_list[i][2])
                            { // success to rent before
            ...
            if (reject_num > 0)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    for (int j = 0; j < station_num; j++)
                    {
                        ...
                        {
                            if (reject_list[k][1] == i && reject_list[k][0] == j + 1)
                            { // bike type that is being checked
                ...
                while (!ifs.eof())
                {
                    ...
                    if (action == "rent")
                    {
                        ...
                        if (my_heap->isEmpty())
                        {
                            ...
                            if (algo.change_bike_type == 10000)
                            { // other types at that station are also empty
